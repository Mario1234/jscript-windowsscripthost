...
function arreglaParseaPagina(paginaHTML1){	
	paginaHTML1.validateOnParse = true;  
	paginaHTML1.async = false;
	var s_paginaHTML2 = solicitadorHTTP1.responseText;
	
	
	//arreglar la pagina
	s_paginaHTML2 = s_paginaHTML2.replace(/\n/g,'');//quitar saltos de linea
...
var solicitadorHTTP1 = new ActiveXObject("Microsoft.XMLHTTP");

solicitadorHTTP1.onreadystatechange = function() {
    if (solicitadorHTTP1.readyState == 4) {
        if (solicitadorHTTP1.status == 200) {
			//var paginaHTML1 = new ActiveXObject('MSXML2.DOMDocument.6.0');// "Msxml2.DOMDocument.6.0");  
            var paginaHTML1 = solicitadorHTTP1.responseXML;
            arreglaParseaPagina(paginaHTML1);
			WScript.Quit(0);
        }
    }
}
